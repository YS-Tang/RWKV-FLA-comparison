...
def StateTransf_rwkv2fla(state_dict_rwkv):
    # 需重命名
    mapping_direct = {
        'receptance.weight': 'r_proj.weight',
        'key.weight': 'k_proj.weight',
        'value.weight': 'v_proj.weight',
        'output.weight': 'o_proj.weight',
        'ln_x.weight': 'g_norm.weight',
        'ln_x.bias': 'g_norm.bias',
    }
    mapping_reshape = {
        'k_k': 'k_k',
        'k_a': 'k_a',
        'w0': 'w_lora.lora.2.bias',
        'v0': 'v_lora.lora.2.bias',
        'a0': 'a_lora.lora.2.bias',
        }
    mapping_transpose = {
        'w1': 'w_lora.lora.0.weight',
        'w2': 'w_lora.lora.2.weight',
        'v1': 'v_lora.lora.0.weight',
        'v2': 'v_lora.lora.2.weight',
        'a1': 'a_lora.lora.0.weight',
        'a2': 'a_lora.lora.2.weight',
        'g1': 'g_lora.lora.0.weight',
        'g2': 'g_lora.lora.2.weight',
    }
    
    state_dict_fla = {}
    # start and end
    for key in ['emb.weight', 'head.weight', 'ln_out.weight', 'ln_out.bias']:
            if key in state_dict_rwkv:
                state_dict_fla[key] = state_dict_rwkv[key]

    # block
    max_block = max(int(key.split('.')[1]) for key in state_dict_rwkv.keys() if key.startswith('blocks.'))

    for i in range(max_block + 1):
        prefix = f'blocks.{i}.'
        # 遍历当前层的所有参数
        for key, value in state_dict_rwkv.items():
            ...
            # 处理 attention 相关的参数
            if key_suffix.startswith('att.'):
                att_key = key_suffix[4:]  # 去掉 'att.'
                
                # 第0层的 v1, v2, v0 不加载
                if i == 0 and att_key in ['v1', 'v2', 'v0']:
                    continue
                
                # 构建新键名
                new_prefix = f'blocks.{i}.att.att.'
                
                # 应用映射规则
                if att_key in mapping_direct:
                    state_dict_fla[new_prefix + mapping_direct[att_key]] = value
                elif att_key in mapping_reshape:
                    state_dict_fla[new_prefix + mapping_reshape[att_key]] = value.view(-1)
                elif att_key in mapping_transpose:
                    state_dict_fla[new_prefix + mapping_transpose[att_key]] = value.T
                else:
                    state_dict_fla[new_prefix + att_key] = value
            else:
                # 非 attention 参数直接复制
                state_dict_fla[key] = value
    
    return state_dict_fla
...
"""
    ...
    elif run_type == 'fla' and init_method == 'share':
        load_dict = StateTransf_rwkv2fla(load_dict)
        model.load_state_dict(load_dict)
