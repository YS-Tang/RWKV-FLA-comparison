...
class RWKV_Tmix_fla(nn.Module):
    def __init__(self, args, layer_id):
        ...
        self.att = RWKV7Attention(
            ...
            num_heads=args.dim_att // args.head_size,
            layer_idx=layer_id,
            num_hidden_layers=args.n_layer,
            value_dim=args.n_embd,
            fuse_norm=False,
        )
        self.att.apply(self.att._initialize_weights) # 强制初始化

    def forward(self, x, v_first):
        o, _, _, v_first = self.att(hidden_states=x, v_first=v_first, use_cache=False)
        return o, v_first
########################################################################################################
...
class Block(nn.Module):
    def __init__(self, args, layer_id):
        ...
        if run_type == "fla":
            self.att = RWKV_Tmix_fla(args, layer_id)
        elif run_type == "rwkv":
